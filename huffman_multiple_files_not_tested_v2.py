...
def read_encodedfile(file, skip):
    path = os.path.join(sys.path[0], file)
    with open(path, "r") as rfile:
        for i in range(skip):
            rfile.readline()
...
def compress(paths):
    ...
    for path in paths:
        ...
        with open('encoded.txt', "a") as wfile:
            char_val=''
            for key, value in frequency.items():
...
def decompress():
    ...
    count = 0
    skip = 0
    with open('encoded.txt', "r") as rfile:
        count = int(rfile.readline())
        skip+=1
    for i in range(count):
        encoded_msg, frequency = read_encodedfile('encoded.txt',skip)
        buildTree(frequency,1)
        file_num = str(i) + '.txt'
        with open(file_num, "w") as wfile:
            decoded_msg=''
...
while 1:
    ...
    if flag == 'e' or flag == 'E':
        ...
        if flag == 'd' or flag == 'D':
            ...
            file = [str(folder+'\\'+x) for x in file]
            print(file)
            encoded_code = {}
            compress(file)
